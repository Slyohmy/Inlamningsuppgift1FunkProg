...
public class RewriteMe {
    ...
    public List<String> getAllQuestionStringsBelongingACategory(Category category){
        return questions.stream().filter(question -> question.category.equals(category)).map(question -> question.question).collect(Collectors.toList());
    }
    ...
    public int getAnswerCandidateFrequncy(String answerCandidate){
        return (int) questions.stream().filter(answer -> answer.getAllAnswers().contains(answerCandidate)).count();
